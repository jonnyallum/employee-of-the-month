...
| [>] | FND-010 | P0 | Eng/Ops | 1 | FND-002 | **Unblocked.** The Android SDK was already installed at `%LOCALAPPDATA%\Android\Sdk` with `android-36`, build-tools 36.1.0, platform-tools, NDK, the emulator and a `Medium_Phone` AVD. Nothing was missing except `ANDROID_HOME`, which is why Expo Doctor could not see it; it is now set at User scope along with `platform-tools` and `emulator` on PATH, so it survives a reboot. Expo Doctor passes 17/17 with the SDK visible, and `expo prebuild --platform android` generates the native project cleanly. Remaining: the debug APK and a screenshot from the emulator. `/android` and `/ios` are now gitignored, because they are generated from `app.json` and committing them would create a second source of truth for native configuration that silently wins. |
