...
| [!] | FND-010 | P0 | Eng/Ops | 1 | FND-002 | **SDK unblocked, native build blocked on one Windows setting.** The Android SDK was already installed with `android-36`, build-tools 36.1.0, platform-tools, NDK 26 and 27, the emulator and an AVD; only `ANDROID_HOME` was missing, which is why Expo Doctor reported it absent. Now set at User scope with `platform-tools` and `emulator` on PATH. Doctor passes 17/17 and `expo prebuild` generates the native project cleanly. The Gradle build compiles Java and Kotlin successfully and then fails in the native step with `ninja: error: manifest 'build.ninja' still dirty after 100 tries`. **Cause: `LongPathsEnabled` is 0 in the registry.** Native object files nest under `.cxx/tools/debug/arm64-v8a/CMakeFiles/...`, which takes absolute paths past the 260-character limit, so ninja cannot stat or write them and never converges. It presents as a loop rather than a path error, which is why it is worth recording. **Needs Jonny:** enable long paths (an administrator registry change, so not something an agent should make), or build from a short path such as `C:\eotm`. Remaining after that: the debug APK, an emulator screenshot and the 16 KB page-size check. |
...
| [>] | INF-011 | P0 | Eng/Sec | 0.5 | INF-002 | Auth hardened earlier; the remaining item is leaked-password protection, now known to be **impossible on the free tier**: the management API returns `402 Payment Required`. It joins backups and the free-tier pause as reasons to move to Pro before closed testing, rather than being a setting somebody forgot. |
...
| [>] | DB-017 | P0 | Eng/Sec | 2 | DB-005..015 | Advisors run against the **hosted** schema, not a local container. Thirty findings, of which twenty-nine are the design being reported back: seven INFO `RLS enabled, no policy` on exactly the tables the threat model closes deliberately, which is stronger than the advisor can see because those tables also have no grant; and twenty-two WARN `signed-in users can execute SECURITY DEFINER function`, which is every guarded function and is the architecture, each covered by role-based tests asserting what it refuses. Both accepted by design and recorded with reasoning. **One real finding:** leaked password protection is disabled and enabling it returns `402 Payment Required`, so it is a Pro-plan feature this free project cannot have. The advisor list therefore cannot reach zero on the current plan. Remaining: performance advisors and a re-run once on Pro. |
| [>] | DB-018 | P0 | Sec | 3 | DB-005..017 | Adversarial structural review run against the **hosted** schema, not a local container. Eleven checks, all clean: no view reaches the ballot table, every `SECURITY DEFINER` function pins `search_path`, no function is callable by `anon`, no function returns anything named nominator, no granted table lacks a policy, every `UPDATE` policy carries `WITH CHECK`, no policy reads `raw_user_meta_data`, every table has RLS, and no policy grants to `PUBLIC`. The only client-writable thing in the entire schema is `profiles.display_name`. One apparent finding, a function matching `voter`, was `get_cycle_turnout` returning `eligible_voters`, a count rather than an identity, so it was my regex rather than a leak. Remaining: behavioural IDOR and JWT-staleness probes against a live session, which need a hosted smoke fixture. |
...
| [>] | PRV-009 | P0 | Product/Eng | 1 | PRV-002..008 | `src/app/privacy.tsx`: confidentiality explanation, the organisation's actual retention policy read from `recognition_settings`, subject export with a download on web, leave organisation, and account deletion behind a typed DELETE confirmation. The confidentiality copy is the same claim `docs/LEGAL_AND_PRIVACY.md` makes and deliberately says confidential rather than anonymous. A pending deletion request replaces the form, so somebody cannot queue a second one from the interface either. **Type-checked, linted and wired, but not yet driven in a browser**: repeated attempts to sign in through the automated browser did not register the click, while the same credentials work at the API, so the blocker is the automation rather than the screen. Needs a manual pass before this is called done. |
