...
| [>] | INF-003 | P0 | Ops/Eng | 1 | INF-002 | Auth mail moved off the built-in Supabase mailer, which allowed only two messages an hour and delivered only to project members. Now sends through Resend on `jonnyai.co.uk`, whose DKIM and SPF records are verified, at `recognition@jonnyai.co.uk` as `Employee of the Month`, one message per address per 60 seconds and 100 per hour project wide. Applied by `scripts/apply-smtp-config.ps1`, which refuses to install a credential Resend rejects. Uses this product's own `RESEND_API_KEY`, a distinct key rather than a copy of a shared one, so another product's rotation cannot break sign-up here. Remaining for completion: send and receive a real synthetic verification message. |
| [ ] | INF-014 | P1 | Eng/Sec | 0.5 | INF-003 | Replace this product's Resend key with one restricted to `Sending access` on `jonnyai.co.uk`. The current key has full access, so it can list and create further Resend API keys, which is more privilege than an SMTP sender needs and more than should sit in a project's auth configuration. Resend cannot narrow an existing key, so this means creating a replacement and re-running `scripts/apply-smtp-config.ps1 -SetPassword`. `apply-smtp-config.ps1` already warns while the key is over-privileged, so the warning disappearing is the evidence. |
...
| [>] | DB-001 | P0 | Eng | 2 | INF-001, PRE-013 | `docs/SCHEMA_THREAT_MODEL.md` drafted: assets ranked by harm, trust boundaries, six actors, a per-table Data API exposure matrix, the guarded function surface with its hardening rules, ten threats with controls and stated residual risk, the six-role test matrix and eleven named tests that must exist. Key decisions: `recognition_nominations` gets no client grant at all rather than a restrictive policy, `participants` is granted as a column subset that withholds `user_id` and `can_vote`, and the admin response truncates timestamps to the day and orders by name rather than time. **Awaiting security reviewer and product owner sign-off**, which is what closes this card and unblocks `DB-002`. |
...
| [ ] | DB-003 | P0 | Eng | 2 | DB-001, DOM-002 | Create recognition cycles, nominations, settings, audit, delivery and privacy tables. Constraints match PRD. |
| [ ] | DB-004 | P0 | Eng/Sec | 2 | DB-002 | Add private membership/role helpers using `auth.uid()` safely. No authorisation uses user metadata. |
| [ ] | DB-005 | P0 | Eng/Sec | 3 | DB-002, DB-004 | Add explicit grants and RLS for identity/organisation tables. Tests cover signed-out, member, admin, owner and other tenant. |
| [ ] | DB-006 | P0 | Eng/Sec | 3 | DB-003, DB-004 | Add explicit grants and RLS for recognition tables. Other users and admins cannot raw-select ballots. |
