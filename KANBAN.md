...
| [>] | AUT-002 | P0 | Eng/Design | 2 | AUT-001, FND-007 | `src/app/sign-in.tsx` plus a session provider and route guard. Sign-in and sign-out work end to end against the local stack, verified in a browser rather than asserted. A failed attempt returns the same wording whether the address is unknown or the password is wrong, so the screen cannot be used to discover which colleagues have accounts. Errors carry `accessibilityRole="alert"` so they are announced rather than left to be noticed. The guard waits for the stored session before redirecting, otherwise a returning user is bounced to sign-in for a frame. Remaining: sign-up, verification resend and recovery. |
